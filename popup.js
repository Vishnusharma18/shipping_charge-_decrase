...
// ─── 10-Trial Activation Gate ────────────────────────────────────────────────
const TRIAL_LIMIT = 10;
const TRIAL_STORAGE_KEY = 'ecomwithnabeel_trial_uses_v1';
let trialUses = 0;
let trialReady = false;

function showActivationGate() {
  const existing = document.getElementById('activationGate');
  if (existing) return;

  const gate = document.createElement('div');
  gate.id = 'activationGate';
  gate.style.cssText = [
  ...
  gate.innerHTML = `
    <div style="max-width:360px">
      ...
      <h2 style="margin:0 0 10px;font-size:22px;color:#111">Trial Finished</h2>
      <p style="margin:0 0 18px;line-height:1.5;color:#555">
        10 free trials have been used. The extension is now locked.
      </p>
      <p style="margin:0 0 18px;font-weight:700;color:#111">
        Contact us for activation.
      </p>
      <button id="activationCloseBtn" style="border:0;border-radius:10px;padding:11px 18px;background:#111;color:#fff;font-weight:700;cursor:pointer">
        Close
      </button>
    </div>`;
  document.body.appendChild(gate);
  document.getElementById('activationCloseBtn').addEventListener('click', () => window.close());
}

function checkTrialBeforeRun() {
  if (!trialReady) return true; // storage read is still initializing
  ...
  trialUses++;
  chrome.storage.local.set({ [TRIAL_STORAGE_KEY]: trialUses });
  return true;
}

chrome.storage.local.get([TRIAL_STORAGE_KEY]).then(data => {
  const n = Number(data[TRIAL_STORAGE_KEY]);
  trialUses = Number.isFinite(n) && n >= 0 ? Math.floor(n) : 0;
  trialReady = true;
  if (trialUses >= TRIAL_LIMIT) showActivationGate();
}).catch(() => {
  // Fail-open only during storage initialization so the original extension
  // remains responsive; subsequent successful reads enforce the gate.
  trialReady = true;
});
...
function isTargetShipping(shipping, target) {
  ...
  return shipping >= target.min && (target.max === null || shipping <= target.max);
}
...
const LIGHT_PALETTE = [
  '#FFFFFF', '#FAFAFA', '#F7F7F7', '#F5F5F5', '#F2F2F2',
  '#F8FBFF', '#F4F9FF', '#F7FBF7', '#FFF9F2', '#FFF7FA',
  '#F6F6FF', '#F9F7FF', '#F2FAFA', '#FCFCFC'
];
const SOFT_PALETTE = [
  '#EAF4FF', '#EAFBF3', '#FFF1E6', '#F4EEFF', '#FFEFF4',
  '#EEF7F7', '#F5F0E8', '#EDF2FF'
];
...
function randomBorderStyle() {
  ...
  return { style: 'solid', thickness, color, css: `${thickness}px solid ${color}` };
}
    ...
    img.onload = () => {
      ...
      if (profile.border) {
        ...
        if (border.thickness > 0) {
          ...
          ctx.strokeRect(half, half, size - border.thickness, size - border.thickness);
        }
      }
      ...
      let dw, dh;
      if (aspect >= 1) { dw = drawSize; dh = drawSize / aspect; }
      else { dh = drawSize; dw = drawSize * aspect; }
      ...
      if (profile.emoji) {
        ...
        const positions = [
          ...
          { x: size * 0.5,  y: size * 0.10 },  // top-center
        ];
      ...
      resolve({ dataUrl: canvas.toDataURL('image/jpeg', 0.92), index });
    };
...
function applyGradientToCanvas(ctx, gradStr, size) {
  ...
  if (gradStr.startsWith('linear')) {
    ...
    gradient = ctx.createLinearGradient(
      size/2 - Math.cos(rad)*size/2, size/2 - Math.sin(rad)*size/2,
      size/2 + Math.cos(rad)*size/2, size/2 + Math.sin(rad)*size/2
    );
  } else {
      ...
      func: async (imgData) => {
        ...
        function readShipping() {
          ...
          for (const sel of [
            ...
            '[id*="shipping"]', '[data-testid*="shipping"]'
          ]) {
        ...
        async function triggerPrice() {
          ...
          const tempVal = String(originalNum > 1 ? originalNum - 1 : originalNum + 1);
        ...
        if (!input) return { shipping: null, error: 'Upload input not found' };

        // Capture the value that belongs to the previous image.
        const oldShipping = readShipping();

        const res = await fetch(imgData);
        const blob = await res.blob();
        const file = new File([blob], 'test_variant.jpg', { type: 'image/jpeg' });
        ...
        // Same fast image-processing timing as the original.
        await new Promise(r => setTimeout(r, 800));
        await triggerPrice();
        await new Promise(r => setTimeout(r, 200));

        // Wait for Meesho's shipping value to actually refresh.
        // If it changes, return immediately. If it remains identical, allow it
        // after the normal fast window so a legitimate same-price result works.
        ...
        while (Date.now() < deadline) {
          ...
          if (value !== null) {
            ...
            // Same value as before: don't immediately trust a stale DOM value.
            // If it stays stable for 1200ms after the refresh trigger, it is
            // reasonable to treat it as the new value too.
        ...
        const finalValue = readShipping();
        return { shipping: finalValue };
      },
      args: [variantDataUrl]
    }, (execResults) => {
...
async function startTargetSearch() {
  ...
  showStatus(`🎯 Target: ₹${target.min}${target.max === null ? '+' : '–₹' + target.max} — searching...`, '');

  // Generate a larger parallel candidate batch.
  // The UI still has no fixed variation-count selector; batches are internal only.
  // Larger batches preserve the original parallel-generation advantage while
  // giving the search a much wider random candidate pool.
  const BATCH_SIZE = 24;
  ...
  reader.onload = async (ev) => {
    ...
    try {
      while (running) {
        ...
        const indexes = Array.from({ length: BATCH_SIZE }, (_, i) => batchStart + i);

        updateProgress(variantCounter, Math.max(variantCounter + 1, batchStart + BATCH_SIZE - 1),
          `🎨 Generating ${BATCH_SIZE} candidates ${batchStart}–${batchStart + BATCH_SIZE - 1}...`);
        ...
        for (const variant of batch) {
          ...
          updateProgress(variantCounter, variantCounter + 1, `🚀 Testing variant #${variant.index}`);

          let shipping = null;
          try {
            shipping = await getShippingForVariant(variant.dataUrl, targetTab.id);
          } catch (err) {
            console.error('[Adeeb Target Shipping] Shipping test error:', err);
          }
          ...
          if (shipping === null) {
            showStatus(`⏳ Variant #${variant.index}: shipping detect nahi hui — next...`, '');
            continue;
          }
          ...
          if (hit) {
            // Stop FIRST. Nothing after this point can start another variant.
            running = false;
      ...
      console.error('[Adeeb Target Shipping] Search error:', err);
...
function showTargetResult(variant, shipping, target) {
  ...
  const targetText = target.max === null ? `₹${target.min}+` : `₹${target.min}–₹${target.max}`;
  document.getElementById('progressFill').style.width = '100%';
  document.getElementById('progressText').textContent = '🎯 Target found!';
  document.getElementById('progressCount').textContent = `${variant.index} tested`;
  grid.innerHTML = `
  ...
  showStatus(`🎯 Target ₹${shipping} found! Image automatically uploaded.`, 'success');
}
...
function displayResults(top6, autoSelectedIndex) {
  ...
  grid.innerHTML = '';
  const medals = ['🥇', '🥈', '🥉', '4', '5', '6'];
  const rankNames = ['rank-1', 'rank-2', 'rank-3', 'rank-4', 'rank-5', 'rank-6'];
  ...
  top6.forEach((item, i) => {
    ...
    const shippingClass = item.shipping === null ? 'shipping-na' :
                          item.shipping <= 50 ? 'shipping-low' :
                          item.shipping <= 100 ? 'shipping-mid' : 'shipping-high';
    ...
    btn.addEventListener('click', async function() {
      ...
      if (targetTab) {
        try {
          ...
          if (variant) {
            freshShipping = await getShippingForVariant(variant.dataUrl, targetTab.id);
          }
        } catch (e) {
          freshShipping = null;
        }
      ...
      const shippingValEl = this.closest('.result-info').querySelector('.shipping-val');
      if (shippingValEl) {
        if (freshShipping !== null) {
          const newDisplay = `₹${freshShipping}`;
          shippingValEl.textContent = newDisplay;
          shippingValEl.className = 'shipping-val ' + (
            freshShipping <= 50 ? 'shipping-low' :
            freshShipping <= 100 ? 'shipping-mid' : 'shipping-high'
          );
  ...
  try {
    ...
    const execution = await chrome.scripting.executeScript({
      ...
      func: async (imgData) => {
        ...
        const res = await fetch(imgData);
        const blob = await res.blob();
        const file = new File([blob], 'selected_variant.jpg', { type: 'image/jpeg' });
        const dt = new DataTransfer();
        dt.items.add(file);
        const nativeFileSetter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'files').set;
    ...
    if (r && r.success) {
      ...
      showStatus('⚠️ Target mil gaya, lekin image upload nahi ho saki.', 'error');
    }
  } catch (e) {
...
// Show remaining trial count in the popup.
function updateTrialIndicator() {
  let el = document.getElementById('trialIndicator');
  if (!el) {
    el = document.createElement('div');
    el.id = 'trialIndicator';
    el.style.cssText = 'margin:8px 0;text-align:center;font:600 12px Arial;color:#666;';
    const target = document.getElementById('generateBtn');
    if (target && target.parentElement) target.parentElement.insertBefore(el, target);
  }
  el.textContent = trialUses >= TRIAL_LIMIT
    ? '🔒 Trial finished — activation required'
    : `Free trials remaining: ${Math.max(0, TRIAL_LIMIT - trialUses)}`;
}

document.addEventListener('DOMContentLoaded', () => {
  updateTrialIndicator();
  const btn = document.getElementById('generateBtn');
  if (btn) btn.addEventListener('click', updateTrialIndicator);
});
